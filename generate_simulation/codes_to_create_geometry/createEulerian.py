...
class EulerianModel():
    ...
    def createPart(self):
        ...
        s.HorizontalDimension(textPoint=(16.3909759521484, -5.15337371826172), value=self.Width, vertex1=s.vertices[0], vertex2=s.vertices[1])
        s.VerticalDimension(textPoint=(-3.11485934257507, 5.61979579925537), value=self.Height, vertex1=s.vertices[9], vertex2=s.vertices[0])
        s.HorizontalDimension(textPoint=(3.8, 3.125), value=2.35, vertex1=s.vertices[9], vertex2=s.vertices[7])
        s.VerticalDimension(textPoint=(5.7988452911377, 0.750127792358398), value=0.525, vertex1=s.vertices[3], vertex2=s.vertices[1])
        # Creating Body
        self.p = mdb.models['Model-1'].Part(dimensionality=THREE_D, name=self.PartName, type=EULERIAN)
        self.p.BaseSolidExtrude(depth=0.02, sketch=s)
    ...
    def createPartition(self):
        ...
        s.DistanceDimension(entity1=s.geometry[14], entity2=s.geometry[6], textPoint=(-1.68507532417297, -0.90773503527832), value=0.2)
        s.DistanceDimension(entity1=s.geometry[13], entity2=s.geometry[6], textPoint=(-1.90749935447693, -0.912672207214355), value=0.35)
        s.DistanceDimension(entity1=s.geometry[15], entity2=s.geometry[6], textPoint=(-2.13980893432617, -0.922546789505005), value=0.5)
        s.DistanceDimension(entity1=s.geometry[17], entity2=s.geometry[6], textPoint=(-2.41166071235657, -1.01141731486511), value=0.625)
        s.DistanceDimension(entity1=s.geometry[16], entity2=s.geometry[6], textPoint=(-2.64878467857361, -0.829298302986145), value=1.0)
        # s.dimensions[3].setValues(value=0.625)
        s.DistanceDimension(entity1=s.geometry[7], entity2=s.geometry[9], textPoint=(-0.843816559906006, 1.32369954838562), value=1.75)
        s.DistanceDimension(entity1=s.geometry[7], entity2=s.geometry[10], textPoint=(-0.478982728118897, 1.47714264645386), value=2.05)
        s.DistanceDimension(entity1=s.geometry[11], entity2=s.geometry[7], textPoint=(-1.00703195869446, 1.64976604237366), value=2.25)
    ...
    def createSetsandSections(self):
        ...
        self.p.Set(cells=self.p.cells.getSequenceFromMask(('[#ffffffff:6 #1f ]', ), ), name=self.EulerDomain)
        # Creating WorkpieceDomain Set 
        # self.p.Set(edges=self.p.cells.getSequenceFromMask(('[#e0fc0e00 ]', ), ), name=self.WorkpieceDomain)
        self.p.Set(cells=self.p.cells.getSequenceFromMask(('[#e0fc0e00 ]', ), ), name=self.WorkpieceDomain)
        # Creating Sections
        self.m.EulerianSection(data={'inconel718-1': 'Inconel718'}, name=self.SectionName)
    ...
    def createMesh(self):
        ...
        self.p.seedPart(deviationFactor=0.1, minSizeFactor=0.1, size=0.02)
