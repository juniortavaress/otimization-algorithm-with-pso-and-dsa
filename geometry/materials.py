...
class Materials():
    ...
    def materialsPart(self):
        ...
        table_damage = (0.04, 0.75, -1.45, 0.04, 0.89, 1260, 25, 0.0001)
        ...
        m.materials['Inconel718'].Density(dependencies=0, distributionType=UNIFORM, fieldName='', table=((mass_density, ), ), temperatureDependency=OFF)
        m.materials['Inconel718'].InelasticHeatFraction(fraction=inelastic_heat_fraction)
        m.materials['Inconel718'].Conductivity(dependencies=0, table=table_conductive, temperatureDependency=ON, type=ISOTROPIC)
        m.materials['Inconel718'].JohnsonCookDamageInitiation(alpha=0.0, definition=MSFLD, dependencies=0, direction=NMORI, feq=10.0, fnn=10.0, fnt=10.0, frequency=1, ks=0.0, numberImperfections=4, omega=1.0, position=CENTROID, table=(table_damage, ), temperatureDependency=OFF, tolerance=0.05)
        ...
        m.materials['EMT210 - Extramet'].SpecificHeat(dependencies=0, law=CONSTANTVOLUME, table=table_specific_heat_emt, temperatureDependency=ON)
